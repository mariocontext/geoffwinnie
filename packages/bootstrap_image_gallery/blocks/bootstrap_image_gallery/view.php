...
.gal_img_span_id'.$bID.' {margin:0!important}
#toggle-fullscreen'.$bID.':active ,#toggle-fullscreen'.$bID.'.btn-active {background: #333 !important}
</style>';

//start slideshow btn
if($startss=="true"){
echo '<button id="start-slideshow'.$bID.'" class="btn btn-large btn-success" data-slideshow="5000" data-target="#modal-gallery'.$bID.'" data-selector="#gallery'.$bID.' a[rel=gallery]">Start Slideshow</button>';
} 
...
echo'<span class="gal_img_span gal_img_span_id'.$bID.'"><a rel="gallery" href="'.$img_link.'" class="gal_img" title ="'.$imgInfo["url"].'" ><img src="'.$thumb_link.'" /></a></span>';
...
if ($downloadbtn == 'true') {
echo '<a class="nonbtn modal-download" target="_blank"> <i class="icon-download"></i><span>Download</span> </a>';
	}
if($startss == "true"){	
 echo '<a class="nonbtn btn-success modal-play modal-slideshow" data-slideshow="5000"> <i class="icon-play icon-white"></i> <span>Slideshow</span> </a>';
 }
 echo '<a class="nonbtn btn-info modal-prev"> <i class="icon-arrow-left icon-white"></i> <span>Previous</span> </a><a class="nonbtn btn-primary modal-next"> <span>Next</span> <i class="icon-arrow-right icon-white"></i> </a>
		</div>
	</div>';
//end modal
