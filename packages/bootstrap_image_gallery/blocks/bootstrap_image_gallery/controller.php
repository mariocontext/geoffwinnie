...
class BootstrapImageGalleryBlockController extends BlockController {
	
	protected $btTable = 'btBootstrapImageGallery';
	protected $btInterfaceWidth = "550";
	protected $btInterfaceHeight = "400";


	protected $btExportFileColumns = array('fID');
	protected $btExportTables = array('btBootstrapImageGallery','btBootstrapImageGalleryImg');
	...
	function loadFileSet(){
		...
		foreach ($files as $f) {
			...
			// find the max height of all the images so BootstrapGallery doesn't bounce around while rotating
	...
	function loadImages(){
		...
		$sql = "SELECT * FROM btBootstrapImageGalleryImg WHERE bID=".intval($this->bID).' ORDER BY '.$sortChoices[$this->playback];
		$db = Loader::db();
	...
	function delete(){
		$db = Loader::db();
		$db->query("DELETE FROM btBootstrapImageGalleryImg WHERE bID=".intval($this->bID));		
		parent::delete();
	}
	...
	function duplicate($nbID) {
		...
		foreach($this->images as $im) {
			$db->Execute('insert into btBootstrapImageGalleryImg (bID, fID, url, duration, fadeDuration, groupSet, position, imgHeight) values (?, ?, ?, ?, ?, ?, ?, ?)', 
	...
	function save($data) { 
		...
		$args['imgheight'] = isset($data['imgheight']) ? trim($data['imgheight']):'75';
		$args['imgwidth'] = isset($data['imgwidth']) ? trim($data['imgwidth']):'75';
		$args['aspectratio'] = isset($data['aspectratio']) ? trim($data['aspectratio']):'false';
		$args['checkimg'] = isset($data['checkimg']) ? trim($data['checkimg']):'true';
		$args['startss'] = isset($data['startss']) ? trim($data['startss']):'true';
		$args['tfull'] = isset($data['tfull']) ? trim($data['tfull']):'true';
		$args['cdnstatus'] = isset($data['cdnstatus']) ? trim($data['cdnstatus']):'false';
		$args['autocdn'] = isset($data['autocdn']) ? trim($data['autocdn']):'false';
		$args['cdnurl'] = isset($data['cdnurl']) ? trim($data['cdnurl']):'true';
		$args['downloadbtn'] = isset($data['downloadbtn']) ? trim($data['downloadbtn']):'true';
				$db = Loader::db();
		...
		if( $data['type'] == 'FILESET' && $data['fsID'] > 0){
			...
			foreach($data['imgFIDs'] as $imgFID){ 
				...
				$db->query("INSERT INTO btBootstrapImageGalleryImg (bID,fID,url,duration,fadeDuration,groupSet,imgHeight,position) values (?,?,?,?,?,?,?,?)",$vals);
				$pos++;
			}
		}
